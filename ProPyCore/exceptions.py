...
class BadRequestError(ProcoreException):
...
def raise_exception(response):
    ...
    if response.status_code == 400:
        raise BadRequestError("400: Bad request", response.text)
    elif response.status_code == 401:
        raise UnauthorizedClientError("401: Wrong client secret or/and refresh token", response.text)

    elif response.status_code == 403:
        raise NoPrivilegeError(
            f"403: Data connection app or permission template does not have access to this endpoint", response.text
        )
